import {
  ...
  ngOnInit(): void {
	const apiEndpoint = 'http://143.110.148.64:5000/download/frf';
  
	this.SlopeAEDSService.getFileDataFromAPI(apiEndpoint).subscribe(
	  urlsData => {
		// Assuming the API response contains newline-separated URLs
		const urlsArray = urlsData.split('\n');
		console.log("MSG: URLs extracted from the API",urlsArray)
		if (urlsArray.length === 0) {
		  console.error('MSG: No URLs found in the API response.');
		  return;
		}
  
		const firstUrl = urlsArray[0]; // You can modify this to loop through all URLs
		console.log("MSG: First URL extracted from the API",firstUrl)

		this.SlopeAEDSService.getFileDataFromAPI(firstUrl).subscribe(
		  data => {
			if (!data) {
			  this.SlopeAEDSService.getFileDataCache().subscribe(cachedData => {
				this.fileData = cachedData;
				this.parseData(cachedData); // Parse cached data
			  });
			} else {
			  this.fileData = data;
			  this.SlopeAEDSService.cacheFileData(data);
			  this.parseData(data); // Parse fetched data
			}
		  },
		  error => {
			console.error('MSG: Error fetching URL data:', error);
		  }
		);
	  },
	  error => {
		console.error('MSG: Error fetching API data:', error);
	  }
	);
  }
